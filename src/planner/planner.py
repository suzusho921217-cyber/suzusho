...
_CONCEPT_BEATS = {
    ...
    "擬人化": (
        "人間のような姿勢でくつろぐ（椅子に腰かける／両前足でマグカップを抱える）",
        "前足で頬杖をつく等、人間っぽい仕草を真顔で続ける",
        "小さな物音にハッとして、急にふつうの猫の動き（四つ足で逃げる／毛づくろい）に戻る",
    ),
    # 力関係の逆転。相手は人間の赤ちゃんではなく、ぬいぐるみ／クッション（児童表現を避ける）。
    "設定反転": (
        "自分よりずっと大きなぬいぐるみ（またはクッション）の前で、見張り役のように堂々と構える",
        "ぬいぐるみが自重でゆっくり傾いてもたれかかり、じりじり押されて後ずさりする",
        (
            "観念してぬいぐるみに寄りかかられたまま一緒に寝そべる。ぬいぐるみの方がボスに見える。"
            "子犬は痛がらず、のびのびした表情"
        ),
    ),
}
# agent-mtg が企画・フック・演出を1つに詰めて追加したタグ。中身は既存企画と同じ。
_CONCEPT_BEATS["人間化×ドアップ冒頭×スロー0.8倍速"] = _CONCEPT_BEATS["擬人化"]
...
_HOOK_HINT = {
    ...
    "スロー入り": "冒頭1〜2秒だけスローモーション（約0.8倍速）、オチの直前から通常速度に戻す",
    "スロー入り+自然光": "窓からのやわらかい自然光の中、冒頭1〜2秒だけスローモーション（約0.8倍速）",
    "スロー冒頭フリーズ＋スローエンド": "冒頭は一瞬静止してからスローで動き出し、ラストもスローで余韻を残す",
    "画面外から侵入": "画面の端から被写体がぬっと入ってくる",
    # 動画内の文字は禁止（Veo の文字は崩れる）。テロップは後処理の担当。
    "テロップ入り（無音理解対応）": "文字は入れない。音なしでも状況が一目で分かる、はっきりした動作と表情で見せる",
    "人間視点のドアップ": "人の目線の高さ・手の届く距離から顔にぐっと寄った一人称構図で始まる（人の顔は映さない）",
    "人間視点のドアップ冒頭": "人の目線の高さ・手の届く距離から顔にぐっと寄った一人称構図で始まる（人の顔は映さない）",
    "人間視点ドアップ+スロー": "人の目線から顔にぐっと寄った一人称構図で、冒頭1〜2秒だけスローモーション（約0.8倍速）",
    "力関係逆転ドアップ": "冒頭で自分より大きなぬいぐるみ／クッションと向かい合うドアップ。大きさの差が一目で分かる構図",
}
...
def render_prompt(
    ...
    _extras = ["画面内の文字・字幕・テロップ", "実在ブランドのロゴ", "ウォーターマーク",
               "人間の顔をはっきり映す", "人間の乳幼児・子ども", "動物が痛がる・苦しむ様子"]
