...
- views_delta（再生数の前日比）の計算ロジック自体は2026-09-14にエンジニアが
  ...
  ★2026-09-14コミット64311b2の内容について: これは「web_search承認待ち」等の
  誤った判定ロジックの修正であり、views_delta の計算コード自体には手を入れて
  いない（元々バグが無く、修正の必要が無いと判断したため）。「この修正で
  計測値自体が直るか」を再度エンジニアに確認する必要はない。views_deltaが
  0になりやすい理由は「実装のバグ」ではなく「低再生数の投稿が多い」こと自体
  なので、追加のエンジニアリング対応を待たずに、この前提で判断してよい。
  ★2026-09-14 実地検証済み（もう繰り返し確認しなくてよい）: views_delta が
  複数日連続で0だった投稿を2本、うちのコードを経由せず YouTube の公開ページ
  （youtube.com/watch?v=<動画ID>）を直接見て実際の再生数と突き合わせた。
  結果、どちらもうちの記録と完全一致（61回/8回）。つまり計測は正確で、単に
  その動画が実際にそれ以上誰にも見られていない（Shorts特有の「最初の数日で
  伸びなければそのまま止まる」という配信側の現象）というだけだった。「views_delta
  復旧待機」「計測障害」は誤りだったと確定済み。「〜が直るまで待つ」という決定は
  二度と出さないこと。一方で同じデータの中で伸び続けている投稿も複数あり
  （例: 1515→1586→1669）、計測の仕組み自体は正常に機能している。
"""
