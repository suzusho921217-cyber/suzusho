...
- 再確認: 2026-09-13(1回目、policy_syncがYouTube Data/Analytics APIのrevision historyフィード新着を検知したための再確認。内容はAPIのスコープ緩和・SLA明記等の技術仕様変更のみで、本セクションの記述に差分なし)
- 再確認: 2026-09-13(2回目、同日中に再度フィード新着検知。今回はData API 2026-09-11付「サムネイル解像度定義の更新(fhd/qhd/uhd対応)」のみで、収益化条件・コンテンツポリシー・AI生成物の開示のいずれにも無関係)
- 再確認: 2026-09-18(3回目、9/12〜9/14の3日連続でフィード新着検知しstaleが解除されないまま滞留、9/15〜9/17の投稿でYouTubeのみ連続HOLDされていたのを検知して対応。内容はData API 2026-09-11/14付「サムネイル画像アップロード仕様(解像度・ファイルサイズ上限)」とReporting API 2026-09-13付「週次リファレンスレポートv1.1(フィールド追加のみ)」で、いずれも技術仕様変更に限られ収益化条件・コンテンツポリシー・AI生成物の開示・性的表現規制に無関係。GCS上の.state/policy_sync.json含めstale解除)
